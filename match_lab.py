...
PLAYER_CONFIGURATIONS = [
    {"name": "Always0", "type": "Always0",},
    {"name": "Always3", "type": "Always3",},
    {"name": "UniformRandom", "type": "UniformRandom",},
    {"name": "Focal5", "type": "Focal5",},
    {"name": "TitForTat", "type": "TitForTat",},
    {"name": "CastigadorInfernal", "type": "CastigadorInfernal",},
    # Optimista: Comieza eligiendo 2, not TFT: Si decide castigar elegira siempre 3
    {"name": "Deterministic_simpletron_optimist_notTfT", "type": "Deterministic_simpletron",},
    # Pesimista: Comieza eligiendo 3
    {"name": "Deterministic_simpletron_pesimist_notTfT", "type": "Deterministic_simpletron", "kwargs": {"pesimist_start": True}},
    # TFT punishment: Al castigar, juega como TFT
    {"name": "Deterministic_simpletron_optimist_TfT", "type": "Deterministic_simpletron", "kwargs": {"tit_for_tat_punishment": True}},
    {"name": "Deterministic_simpletron_pesimist_notTfT", "type": "Deterministic_simpletron", "kwargs": {"pesimist_start": True, "tit_for_tat_punishment": True}},
    {"name": "Permisive_TFT_patience_3", "type": "PermissiveTitForTat",},
    {"name": "Permisive_TFT_patience_5", "type": "PermissiveTitForTat", "kwargs": {"initial_patience": 5,}},
    {"name": "Permisive_TFT_patience_10", "type": "PermissiveTitForTat", "kwargs": {"initial_patience": 10,}},
    {"name": "GrimTrigger", "type": "GrimTrigger",},
    # TFT pero perdona con cierta probabilidad
    {"name": "GenerousTitForTat_generous_p_0.1", "type": "GenerousTitForTat", "kwargs": {"prob_generosidad": 0.1}},
    {"name": "GenerousTitForTat_generous_p_0.25", "type": "GenerousTitForTat", "kwargs": {"prob_generosidad": 0.25}},
    {"name": "GenerousTitForTat_generous_p_0.5", "type": "GenerousTitForTat", "kwargs": {"prob_generosidad": 0.5}},
    {"name": "GenerousTitForTat_generous_p_0.75", "type": "GenerousTitForTat", "kwargs": {"prob_generosidad": 0.75}},
    {"name": "ContriteTitForTat", "type": "ContriteTitForTat",},
    # La version determinista cambia de cooperar a rivalizar si no obtiene un beneficio
    {"name": "AdaptivePavlov_deterministic", "type": "AdaptivePavlov",},
    # La version random tiene un 50% de posibilidades de cambiar de estrategia o no
    {"name": "AdaptivePavlov_random", "type": "AdaptivePavlov","kwargs": {"shift_strategy": "random"}},
    # La siguiente version siempre coopera
    {"name": "AdaptivePavlov_random", "type": "AdaptivePavlov","kwargs": {"shift_strategy": "always_coop"}},
    {"name": "Detective", "type": "Detective",},
]
            ...
            for player_name, player_object in all_agents.items():
                for ranking_player_object, ranking_reward in evolution.ranking.items():
                    ...
                    if ranking_player_object.name == player_name:
                        ...
                        new_row = {"error_prob": error_p, "n_repetitions": repetitions, "agent_name": player_name, "reward": reward}
                        rows.append(new_row)
            # Obtenemos la informacion de los enfrentamientos por parejas
            head_to_head_data = pd.concat([head_to_head_data, evolution.get_head_to_head_rewards()])
...
grouped = (
    ranking_data
    .groupby(['error_prob', 'n_repetitions', 'agent_name'])['reward']
...
best_models = (
    grouped
    .groupby(['error_prob', 'n_repetitions'], group_keys=False)
    .apply(top3)
)
...
for (err, reps), subdf in best_models.groupby(['error_prob', 'n_repetitions']):
    ...
    for _, row in subdf.iterrows():
        print(f"    - {row['agent_name']}: mean={row['mean']:.4f}, std={row['std']:.4f}, min={row['min']:.4f}, max={row['max']:.4f}")
...
dfA = head_to_head_data[['agent_A','reward_A']].rename(columns={'agent_A':'agent','reward_A':'reward'})
dfB = head_to_head_data[['agent_B','reward_B']].rename(columns={'agent_B':'agent','reward_B':'reward'})
